...
SchemaNode 是一个 Tree 结构的数据，由节点、子节点和节点上的属性组成。正如 DOM 的结构一样，
SchemaNode 的结构也反映了最后渲染的页面结构。不同类型的节点由 `type` 字段区分，不同类型的节点
的属性也有所差异。
...
此节点为外部引用节点，在节点上记录了外部 Schema 的 ID，可以通过此节点实现 Schema 的组合。

| 名称       | Required | Type         | 描述                                                                                               |
| :--------- | -------- | ------------ | -------------------------------------------------------------------------------------------------- |
| `type`     | 是       | `string`     | 值为 `ref-node`                                                                                    |
| `schemaID` | 是       | `string`     | 被引用的 Schema ID                                                                                 |
| `fallback` | 否       | `SchemaNode` | 当对应的 schema 还有没加载之前，在页面中渲染的内容，可选                                           |
