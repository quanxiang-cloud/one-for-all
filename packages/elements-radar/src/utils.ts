...
export function getReport(elements: HTMLElement[], root: HTMLElement): Promise<Report> {
  return new Promise((resolve) => {
    const observer = new IntersectionObserver(
      (entries) => {
        const pairs: Array<[HTMLElement, { raw: DOMRectReadOnly; rect: Rect }]> = entries.map(
          ({ target, boundingClientRect, rootBounds }) => {
            const rect: Rect = calcRect(boundingClientRect, rootBounds);
            return [target as HTMLElement, { rect, raw: boundingClientRect }];
          },
        );
